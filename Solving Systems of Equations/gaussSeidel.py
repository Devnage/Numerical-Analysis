...
def main() -> None:
    ...
    gaussSeidel(A, b, x0, error_tolerance, max_iterations)
    
def gaussSeidel(A: np.ndarray, b: np.ndarray, x0: np.ndarray, errorTol: float = 1e-6, maxIter: int = 100) -> None:
    table = []
    curr_x = x0
    ...
    if max(np.abs(np.linalg.eigvals(Tg))) >= 1:
    ...
    print(f"Asymptotic error constant upper bound: {np.linalg.norm(Tg, np.inf)}")
